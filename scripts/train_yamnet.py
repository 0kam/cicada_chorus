...
import omegaconf
cfg = omegaconf.OmegaConf.load('/home/okamoto/cicada_chorus/scripts/yamnet/config.yaml')

@hydra.main(config_path="yamnet", config_name="config")
def train(cfg: DictConfig):
    model = YAMNetClassifier(cfg)
    mlflow.set_tracking_uri("file:// /home/okamoto/cicada_chorus/mlruns")
    mlflow.set_experiment(cfg.mlflow.experiment_name)
    best_val_f1_mean = 0.0
    with mlflow.start_run(run_name="{}_{}".format(cfg.model.model_name, datetime.now().strftime("%Y%m%d%H%M%S"))):
